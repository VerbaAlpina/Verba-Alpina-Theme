...
<style type="text/css">
html { 
  background: url(<?php echo get_site_url(1);?>/wp-content/uploads/ProdottiTipici-FormaggioBitto_SalaturaDellaMascherpa677301.jpg) no-repeat center center fixed; 
...
<script type="text/javascript">
	...
	function adjustSizesRelativeToWindow (){
		...
		if(prop <= 1){ //Adjust to browser width
			jQuery("#logoSVG").css({"width": (percX * widthW), "height" : (percX * widthW * logo_h / logo_w), "left": (left_dist * widthW), "top": (top_dist * heightW)});
		}
		else { //Adjust to browser height
			prop = 1 / prop;
			jQuery("#logoSVG").css({"width": (percY * heightW * logo_w / logo_h) , "height": (percY * heightW), "top": (top_dist * heightW), "left": (left_dist * widthW)});
		}
	}
</script>
